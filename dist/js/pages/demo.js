...
//-----------------
    ...
    var baroption = {
        ...
        maintainAspectRatio: true,
        //String - A tooltip template
        tooltipTemplate: "<%=value %> MB",
    };

    var ctx3 = $("#barchart").get(0).getContext("2d");
    var myBarChart = new Chart(ctx3).Bar(bardata, baroption);

    var ctx4 = $("#piechart").get(0).getContext("2d");
    var myPieChart;

    $('#tab3').on('shown.bs.tab', function (e) {
        myPieChart.destroy();
        myBarChart = new Chart(ctx3).Bar(bardata , baroption);
    });

    $('#tab4').on('shown.bs.tab', function (e) {
        myBarChart.destroy();
        myPieChart = new Chart(ctx4).Doughnut(PieData, pieOptions);
    });    
